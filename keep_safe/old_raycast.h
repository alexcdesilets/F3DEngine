...
namespace RayCast {
    ...
    void drawBaseWorld() {
        ...
        // std::vector<QuadData> imageDefinition;
        ...
        if (horizonCenterY > 0.0) {
            ...
            #pragma omp parallel
            {
                // std::vector<QuadData> localQuads;
                // localQuads.reserve(numRays * yScans);

                #pragma omp for nowait schedule(dynamic)
                for (int y = yScans; y > 0; --y) {
                    for (int x = 0; x < numRays; ++x) {
                        ...
                        imageBuffer.fillRect((int)x1, (int)y1, (int)x2, (int)y2, color);
                    }
                }
    ...
    struct rayPack {
        ...
        double distance3d = -vec2::_inf, verticalOffset = -vec2::_inf;
            ...
            for (int x = 0; x < rays.size(); ++x) {
                ...
                for (const rayPack& obj : view) {
                    ...
                    if (obj.isDangling) {
                        ...
                        if (obj.pairPartnerIndex >= 0 && obj.pairPartnerIndex < view.size()) {
